...
const STATUS_CONFIG = {
  ...
  CANCELLED:       { label: 'Cancelled',       color: '#ef4444', bg: 'rgba(239,68,68,0.12)',  icon: XCircle,       desc: 'Booked then cancelled — all bookings cancelled' },
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [tree, gen, testReq] = await Promise.all([
        getSegmentationTree({ businessType }),
        getGeneralSegment({ businessType }).catch(() => ({ data: null })),
