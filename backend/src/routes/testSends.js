...
router.get('/send-log', async (req, res, next) => {
  try {
    ...
    const { status, email, dayNumber, source, dateFrom, dateTo, subscriptionStatus, journeyId, nodeId, channel } = req.query;

    const result = await SendTrackService.getLog({ page, limit, status, email, dayNumber, source, dateFrom, dateTo, subscriptionStatus, journeyId, nodeId, channel });
