...
export async function refreshJourney(journeyId) {
  ...
  // 1) base: sent / unique recipients (entries) / sends_today (Dubai day)
  const { rows: baseRows } = await db.query(`
    SELECT node_id,
      COUNT(*) FILTER (WHERE status NOT IN ('failed','queued'))            AS sent,
  ...
  const ensure = (id) => {
    if (!map.has(id)) map.set(id, {
      node_id: id, sent: 0, entries: 0, sends_today: 0, delivered: 0, bounced: 0,
      opened: 0, human_opened: 0, clicked: 0, human_clicked: 0, landed: 0, gtm_events: 0,
    });
    return map.get(id);
  };
  for (const r of baseRows)   { const n = ensure(r.node_id); n.sent = +r.sent||0; n.entries = +r.entries||0; n.sends_today = +r.sends_today||0; }
  ...
  const rollup = {
    node_id: ROLLUP_KEY,
    sent: sum('sent'), entries: lifecycle.entries, sends_today: sum('sends_today'),
    ...
    landed: sum('landed'), gtm_events: sum('gtm_events'),
  };
    ...
    for (const n of rows) {
      ...
      await client.query(`
        INSERT INTO journey_node_stats (
          ...
          opened, human_opened, clicked, human_clicked, landed, gtm_events,
      ...
      `, [
        ...
        n.opened, n.human_opened, n.clicked, n.human_clicked, n.landed, n.gtm_events,
        botWindowSec(),
      ]);
    }
